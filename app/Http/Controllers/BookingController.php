...
class BookingController extends Controller
{
    ...
    public function member_booking(BookingFormRequest $request)
    {
        ...
        $bookings_check_in = Booking::where('check_in', $request->get('check_in'))->get();
        $bookings_check_out = Booking::where('check_out', $request->get('check_out'))->get();
        $mess = "";

        if (count($bookings_check_in) || count($bookings_check_out)) {

            $mess = "Ngày này đã được đặt";
